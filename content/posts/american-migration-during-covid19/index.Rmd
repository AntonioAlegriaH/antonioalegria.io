...
Durante la pandemia de COVID-19 existieron muchos movimientos de personas en USA. 
existen muchas hipótesis que darían explicación a estas mudanzas:

* Movimiento a lugares más baratos. 
* Migración a estados con tasas bajas de impuestos
* Planeación familiar
* Un incremento en empresas que adoptaron esquemas de trabajo remoto

Cualquiera que fuese una razón, lo cierto es que el movimiento de personas se podría afectar el panorama electoral a corto y largo plazo. 

## Data source: 

Trabajaremos con datos de estimación anual de residentes del [U.S. Census Bureau: us census vintage 2021](https://www.census.gov/data/tables/time-series/demo/popest/2020s-total-cities-and-towns.html) para calcular el **porcentaje de cambio poblacional de cada estado durante el periodo: 2020 - 2021** 
...
datatable(data_perc_change, 
          options = list(
            initComplete = JS("function(settings, json) {",
                            "$(this.api().table().header()).css({'background-color': '#CFDDD4',
                            'color': '#212529'});",
                            "}"
              ),
